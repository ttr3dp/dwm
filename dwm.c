...
enum { NetSupported, NetWMName, NetWMState, NetWMCheck,
       ...
       NetClientList, NetClientListStacking, NetLast }; /* EWMH atoms */
...
struct Client {
	...
	int bw, oldbw;
	unsigned int tags;
	int isfixed, isfloating, isurgent, neverfocus, oldstate, isfullscreen, fakefullscreen, isterminal, noswallow, issticky ;
	int alwaysontop;
	pid_t pid;
	Client *next;
	Client *snext;
...
togglefloating(const Arg *arg)
{
	...
	if (selmon->sel->isfloating)
		...
		selmon->sel->alwaysontop = 0; /* disabled, turn this off too */
	arrange(c->mon);
}
